...
def draw_game():
    ...
    pygame.draw.rect(screen, BLACK, frog)  # Frog is now black for visibility
def move_objects():
    for car in cars:
        car.x += car_speed
        if car.x > WIDTH:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_UP:
                frog.y = max(0, frog.y - frog_size)
            elif event.key == pygame.K_DOWN:
                frog.y = min(HEIGHT - frog_size, frog.y + frog_size)
            elif event.key == pygame.K_LEFT:
                frog.x = max(0, frog.x - frog_size)
            elif event.key == pygame.K_RIGHT:
                frog.x = min(WIDTH - frog_size, frog.x + frog_size)
